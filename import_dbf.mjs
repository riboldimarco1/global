...
async function importBancos() {
  console.log('\n=== Importing BANCOS ===');
  const dbf = await DBFFile.open('attached_assets/bancos.DBF');
  ...
  const columns = ['fecha', 'monto', 'monto_dolares', 'saldo', 'saldo_conciliado', 'numero', 'operacion', 'descripcion', 'conciliado', 'utility', 'banco', 'operador', 'propietario', 'relacionado', 'codigoauto', 'codrel'];
  ...
  for (const r of records) {
    batch.push([
      ...
      r.SALDOCONCI,
      r.NUMERO,
      sanitize(r.OPERACION),
      sanitize(r.DESCRIPCIO),
      r.CONCILIADO,
      r.UTILITY,
      sanitize(r.BANCO),
      sanitize(r.TIPOOP),
      sanitize(r.PROP),
      r.RELAZ,
      sanitize(r.CODIGOAUTO),
      sanitize(r.CODREL)
    ]);
    
    if (batch.length >= BATCH_SIZE) {
      await batchInsert('bancos', columns, batch);
      count += batch.length;
      batch = [];
      if (count % 50000 === 0) console.log(`  Imported ${count} records...`);
    }
  }
...
async function importAdministracion() {
  ...
  for (const r of records) {
    batch.push([
      ...
      r.MONTO,
      r.MONTODOL,
      sanitize(r.UNIDADDEPR),
      r.CAPITAL,
      r.UTILITY,
      sanitize(r.FORMADEPAG),
      sanitize(r.PRODUCTO),
      r.CANTIDAD,
      sanitize(r.INSUMO),
      r.COMPROBANT ? String(r.COMPROBANT) : null,
      sanitize(r.PROVEEDOR),
      sanitize(r.CLIENTE),
      sanitize(r.PERSONALDE),
      sanitize(r.ACTIVIDAD),
      sanitize(r.PROP),
      sanitize(r.UNIDADDEME),
      r.RELAZ,
      sanitize(r.CODIGOAUTO),
      sanitize(r.CODREL)
    ]);
    ...
    if (batch.length >= BATCH_SIZE) {
      await batchInsert('administracion', columns, batch);
      count += batch.length;
      batch = [];
      if (count % 50000 === 0) console.log(`  Imported ${count} records...`);
    }
...
async function importParametros() {
  ...
  for (const r of records) {
    ...
    if (batch.length >= BATCH_SIZE) {
      await batchInsert('parametros', columns, batch);
      count += batch.length;
      batch = [];
    }
  }
...
async function importCheques() {
  ...
  for (const r of records) {
    ...
    if (batch.length >= BATCH_SIZE) {
      await batchInsert('cheques', columns, batch);
      count += batch.length;
      batch = [];
    }
  }
...
async function importAlmacen() {
  ...
  for (const r of records) {
    batch.push([
      ...
      sanitize(r.CODIGOAUTO),
      sanitize(r.CODREL),
      sanitize(r.CATEGORIA),
      sanitize(r.PROP)
    ]);
    
    if (batch.length >= BATCH_SIZE) {
      await batchInsert('almacen', columns, batch);
      count += batch.length;
      batch = [];
    }
  }
...
async function importTransferencias() {
  ...
  for (const r of records) {
    batch.push([
      ...
      sanitize(r.PROP),
      sanitize(r.RIFCED),
      sanitize(r.NUMCUENTA),
      sanitize(r.EMAIL)
    ]);
    
    if (batch.length >= BATCH_SIZE) {
      await batchInsert('transferencias', columns, batch);
      count += batch.length;
      batch = [];
    }
  }
...
async function main() {
  console.log('Starting DBF import (batch mode)...');
  console.log('====================================');
  ...
  try {
    ...
    const counts = await pool.query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM administracion) as administracion,
        (SELECT COUNT(*) FROM parametros) as parametros,
        (SELECT COUNT(*) FROM cheques) as cheques,
        (SELECT COUNT(*) FROM almacen) as almacen,
