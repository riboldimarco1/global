...
export default function MyFiltroDeUnidad({
  ...
  const hasAllUnidades = allowed.length > 0 && unidades.length > 0 && unidades.every(u => allowed.includes(u.nombre));
  const hasRestrictions = !isAdmin && !hasAllUnidades && allowed.length > 0;
