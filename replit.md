...
# System Architecture
The system employs a client-server architecture. The frontend is built with React and TypeScript, using Wouter for routing and TanStack React Query for state management with optimistic UI updates. React Hook Form and Zod handle form validation. jsPDF is used for client-side PDF generation. The application supports PWA auto-updates via a service worker and real-time data synchronization with WebSockets.
