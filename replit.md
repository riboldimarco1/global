...
This project is an administrative control system for agricultural management, designed to enhance operational efficiency and facilitate data-driven decisions. It features a modular user interface, integrated data handling, robust user permission management, and flexible access across eight core modules: Parameters, Administration, Banks, Checks, Harvest, Warehouse, Transfers, and Agrodata. The system aims to provide a comprehensive and user-friendly platform for managing agricultural activities while ensuring data integrity and supporting business growth.
...
### Frontend
The frontend is built with React and TypeScript, using Wouter for routing and TanStack React Query for data management. Forms are handled with React Hook Form and Zod for validation. UI components are developed using shadcn/ui (based on Radix UI) and styled with Tailwind CSS, drawing inspiration from Material Design 3. Client-side PDF generation is managed by jsPDF.

### Backend
The backend is a Node.js Express.js application, written in TypeScript (ES modules), providing RESTful APIs. Drizzle ORM is used for database interactions, with Zod for data validation.

### Data Storage
PostgreSQL serves as the primary database, with its schema defined and managed in `shared/schema.ts`.

### Key Design Patterns
- **Generic CRUD API**: A unified API endpoint (`/api/:tableName`) for standard create, read, update, and delete operations.
- **Shared Schema**: Centralized definitions for database types and validation schemas ensuring consistency between frontend and backend.
- **Storage Interface**: An `IStorage` interface in `server/storage.ts` abstracts database operations, allowing for flexible data source changes.
- **PWA Auto-Update**: A service worker implements dynamic caching and automatic application updates for a seamless user experience.
- **Real-time Sync**: WebSockets, managed via `use-realtime-sync.ts`, enable live data updates across clients.
- **Optimistic UI Updates**: `useTableMutation` hooks are employed for CRUD operations, providing immediate user feedback through optimistic UI updates.
- **`MyDebug` Window**: A dedicated floating debug window (`client/src/pages/MyDebug.tsx`) offers visibility into API calls and errors for development and troubleshooting.
