...
export async function registerRoutes(
  ...
  // [ADMIN] Gráficas de administración (facturas, nómina, ventas)
  app.get("/api/administracion/graficas", async (req, res) => {
    try {
      const { unidad, fechaInicio, fechaFin } = req.query;
      
      let whereClause = sql`WHERE tipo IN ('facturas', 'nomina', 'ventas')`;
      ...
      const lineResult = await db.execute(sql`
        SELECT 
          SUBSTR(fecha, 6, 2) || '/' || SUBSTR(fecha, 3, 2) as mes_key,
        ...
        GROUP BY SUBSTR(fecha, 6, 2) || '/' || SUBSTR(fecha, 3, 2), SUBSTR(fecha, 6, 2), SUBSTR(fecha, 3, 2), tipo
        ORDER BY aa ASC, mm ASC
      `);

      const monthMap: Record<string, { facturas: number; nomina: number; ventas: number }> = {};
      const monthOrder: string[] = [];
      for (const row of lineResult.rows as any[]) {
        const key = row.mes_key;
        if (!key) continue;
        if (!monthMap[key]) {
          monthMap[key] = { facturas: 0, nomina: 0, ventas: 0 };
          monthOrder.push(key);
        }
        const total = parseFloat(row.total) || 0;
        if (row.tipo === "facturas") monthMap[key].facturas += total;
        else if (row.tipo === "nomina") monthMap[key].nomina += total;
        else if (row.tipo === "ventas") monthMap[key].ventas += total;
      }

      const sortedMonths = monthOrder.sort((a, b) => {
        const [mmA, aaA] = a.split("/");
        const [mmB, aaB] = b.split("/");
        if (aaA !== aaB) return aaA.localeCompare(aaB);
        return mmA.localeCompare(mmB);
      });

      const lineData = sortedMonths.map(mes => ({
        mes,
        facturas: Math.round(monthMap[mes].facturas * 100) / 100,
        nomina: Math.round(monthMap[mes].nomina * 100) / 100,
        ventas: Math.round(monthMap[mes].ventas * 100) / 100,
        resultado: Math.round((monthMap[mes].ventas - monthMap[mes].facturas - monthMap[mes].nomina) * 100) / 100,
      }));
      ...
      const insumoResult = await db.execute(sql`
        ...
        ORDER BY value DESC
      `);
      const insumoData = (insumoResult.rows as any[]).map(r => ({
        name: r.name,
      ...
      const actividadResult = await db.execute(sql`
        ...
        ORDER BY value DESC
      `);
      const actividadData = (actividadResult.rows as any[]).map(r => ({
        name: r.name,
        value: Math.round((parseFloat(r.value) || 0) * 100) / 100,
      }));

      res.json({ lineData, insumoData, actividadData });
    } catch (error) {
      console.error("[GET /api/administracion/graficas] Error:", error);
