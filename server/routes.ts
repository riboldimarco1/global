...
export async function registerRoutes(
  ...
  // Endpoint para importar datos de DBF desde un archivo ZIP usando script Python
  // IMPORTANT: Must be defined before the generic /:tableName route
  app.post("/api/import-dbf", upload.single("file"), async (req, res) => {
    try {
      ...
      console.log("[import-dbf] ZIP saved to:", zipPath);
      
      // Run Python script
      const scriptPath = path.join(process.cwd(), "scripts", "import_dbf_global.py");
      ...
      pythonProcess.stdout.on("data", (data) => {
        ...
        console.log("[import-dbf]", data.toString());
      });
      
      pythonProcess.stderr.on("data", (data) => {
        stderr += data.toString();
        console.error("[import-dbf error]", data.toString());
      });
      ...
      pythonProcess.on("close", async (code) => {
        ...
        if (code !== 0) {
          console.error("[import-dbf] Process exited with code:", code);
          return res.status(500).json({ error: stderr || "Error en importación" });
        }
        
        // Extract JSON results from stdout
        const jsonMarker = "__JSON_RESULTS__";
        const jsonIndex = stdout.indexOf(jsonMarker);
        let results = [];
        
        if (jsonIndex !== -1) {
          try {
            const jsonStr = stdout.substring(jsonIndex + jsonMarker.length).trim();
            results = JSON.parse(jsonStr);
          } catch (e) {
            console.error("[import-dbf] Error parsing results:", e);
          }
        }
        
        // Broadcast updates for all tables
        broadcast("parametros_updated");
        broadcast("bancos_updated");
        broadcast("administracion_updated");
