...
export async function registerRoutes(
  ...
  // =============================================
  // ARRIME MODULE ROUTES
  // =============================================

  // Arrime Registros
  app.get("/api/arrime/registros", async (req, res) => {
    try {
      const result = await db.execute(sql`SELECT * FROM arrime_registros ORDER BY fecha DESC`);
      res.json(result.rows);
    } catch (error) {
  ...
  app.post("/api/arrime/registros", async (req, res) => {
    try {
      const { fecha, central, cantidad, grado, finca, remesa } = req.body;
      if (!fecha || !central || cantidad === undefined) {
        return res.status(400).json({ error: "Datos incompletos" });
      }
      const result = await db.execute(sql`
        INSERT INTO arrime_registros (fecha, central, cantidad, grado, finca, remesa)
        VALUES (${fecha}, ${central}, ${cantidad}, ${grado}, ${finca}, ${remesa})
        RETURNING *
      `);
      broadcast("arrime_registros_updated");
      res.status(201).json(result.rows[0]);
    } catch (error) {
      console.error("Error al crear registro de arrime:", error);
      res.status(500).json({ error: "Error al crear registro" });
    }
  });

  app.put("/api/arrime/registros/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const { fecha, central, cantidad, grado, finca, remesa } = req.body;
      const result = await db.execute(sql`
        UPDATE arrime_registros 
        SET fecha = ${fecha}, central = ${central}, cantidad = ${cantidad}, 
            grado = ${grado}, finca = ${finca}, remesa = ${remesa}
        WHERE id = ${id}
        RETURNING *
      `);
      if (result.rows.length === 0) {
      ...
      console.error("Error al actualizar registro de arrime:", error);
      res.status(500).json({ error: "Error al actualizar registro" });
    }
  });

  app.delete("/api/arrime/registros/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const result = await db.execute(sql`DELETE FROM arrime_registros WHERE id = ${id} RETURNING id`);
      if (result.rows.length === 0) {
        return res.status(404).json({ error: "Registro no encontrado" });
      }
      broadcast("arrime_registros_updated");
      ...
      console.error("Error al eliminar registro de arrime:", error);
      res.status(500).json({ error: "Error al eliminar registro" });
    }
  });

  app.delete("/api/arrime/registros", async (req, res) => {
    try {
      await db.execute(sql`DELETE FROM arrime_registros`);
      broadcast("arrime_registros_updated");
      res.status(204).send();
    } catch (error) {
      console.error("Error al eliminar todos los registros de arrime:", error);
      res.status(500).json({ error: "Error al eliminar registros" });
    }
  });

  // Arrime Centrales
  app.get("/api/arrime/centrales", async (req, res) => {
    try {
      const result = await db.execute(sql`SELECT * FROM arrime_centrales ORDER BY orden, nombre`);
      res.json(result.rows);
    } catch (error) {
      console.error("Error al obtener centrales de arrime:", error);
      res.status(500).json({ error: "Error al obtener centrales" });
    }
  });

  app.post("/api/arrime/centrales", async (req, res) => {
    try {
      ...
      const result = await db.execute(sql`
        INSERT INTO arrime_centrales (nombre, color, orden)
        VALUES (${nombre}, ${color || '#3b82f6'}, ${orden || 0})
        RETURNING *
      `);
      broadcast("arrime_centrales_updated");
      res.status(201).json(result.rows[0]);
    } catch (error: any) {
      if (error?.code === '23505') {
        return res.status(400).json({ error: "Ya existe una central con ese nombre" });
      }
      console.error("Error al crear central de arrime:", error);
      res.status(500).json({ error: "Error al crear central" });
    }
  });

  app.put("/api/arrime/centrales/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const { nombre, color, orden } = req.body;
      const result = await db.execute(sql`
        UPDATE arrime_centrales 
        SET nombre = ${nombre}, color = ${color}, orden = ${orden}
        WHERE id = ${id}
        RETURNING *
      `);
      if (result.rows.length === 0) {
      ...
      console.error("Error al actualizar central de arrime:", error);
      res.status(500).json({ error: "Error al actualizar central" });
    }
  });

  app.delete("/api/arrime/centrales/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const result = await db.execute(sql`DELETE FROM arrime_centrales WHERE id = ${id} RETURNING id`);
      if (result.rows.length === 0) {
        return res.status(404).json({ error: "Central no encontrada" });
      }
      broadcast("arrime_centrales_updated");
      ...
      console.error("Error al eliminar central de arrime:", error);
      res.status(500).json({ error: "Error al eliminar central" });
    }
  });

  // Arrime Fincas
  app.get("/api/arrime/fincas", async (req, res) => {
    try {
      const result = await db.execute(sql`SELECT * FROM arrime_fincas ORDER BY orden, nombre`);
      res.json(result.rows);
    } catch (error) {
      console.error("Error al obtener fincas de arrime:", error);
      res.status(500).json({ error: "Error al obtener fincas" });
    }
  });

  app.post("/api/arrime/fincas", async (req, res) => {
    try {
      ...
      const result = await db.execute(sql`
        INSERT INTO arrime_fincas (nombre, orden)
        VALUES (${nombre}, ${orden || 0})
        RETURNING *
      `);
      broadcast("arrime_fincas_updated");
      res.status(201).json(result.rows[0]);
    } catch (error: any) {
      if (error?.code === '23505') {
        return res.status(400).json({ error: "Ya existe una finca con ese nombre" });
      }
      console.error("Error al crear finca de arrime:", error);
      res.status(500).json({ error: "Error al crear finca" });
    }
  });

  app.put("/api/arrime/fincas/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const { nombre, orden } = req.body;
      const result = await db.execute(sql`
        UPDATE arrime_fincas 
      ...
      console.error("Error al actualizar finca de arrime:", error);
      res.status(500).json({ error: "Error al actualizar finca" });
    }
  });

  app.delete("/api/arrime/fincas/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const result = await db.execute(sql`DELETE FROM arrime_fincas WHERE id = ${id} RETURNING id`);
      if (result.rows.length === 0) {
        return res.status(404).json({ error: "Finca no encontrada" });
      }
      broadcast("arrime_fincas_updated");
      ...
      console.error("Error al eliminar finca de arrime:", error);
      res.status(500).json({ error: "Error al eliminar finca" });
    }
  });

  // Arrime Backups
  app.get("/api/arrime/backups", async (req, res) => {
    try {
      const result = await db.execute(sql`SELECT id, nombre, fecha FROM arrime_backups ORDER BY fecha DESC`);
      res.json(result.rows);
    } catch (error) {
      console.error("Error al obtener backups de arrime:", error);
      res.status(500).json({ error: "Error al obtener backups" });
    }
  });

  app.post("/api/arrime/backups", async (req, res) => {
    try {
      const { nombre } = req.body;
      const fecha = new Date().toISOString();
      
      // Get all current data
      const registrosResult = await db.execute(sql`SELECT * FROM arrime_registros`);
      const centralesResult = await db.execute(sql`SELECT * FROM arrime_centrales`);
      const fincasResult = await db.execute(sql`SELECT * FROM arrime_fincas`);
      
      const datos = JSON.stringify({
        registros: registrosResult.rows,
        centrales: centralesResult.rows,
        fincas: fincasResult.rows
      });
      
      const result = await db.execute(sql`
        INSERT INTO arrime_backups (nombre, fecha, datos)
        VALUES (${nombre || 'Respaldo ' + fecha}, ${fecha}, ${datos})
        RETURNING id, nombre, fecha
      `);
      
      res.status(201).json({
        ...result.rows[0],
        registrosCount: registrosResult.rows.length
      });
    } catch (error) {
      console.error("Error al crear backup de arrime:", error);
      res.status(500).json({ error: "Error al crear backup" });
    }
  });

  app.post("/api/arrime/backups/:id/restore", async (req, res) => {
    try {
      const { id } = req.params;
      const backupResult = await db.execute(sql`SELECT datos FROM arrime_backups WHERE id = ${id}`);
      
      if (backupResult.rows.length === 0) {
        return res.status(404).json({ error: "Backup no encontrado" });
      }
      
      const datos = JSON.parse((backupResult.rows[0] as any).datos);
      
      // Clear and restore
      await db.execute(sql`DELETE FROM arrime_registros`);
      await db.execute(sql`DELETE FROM arrime_centrales`);
      await db.execute(sql`DELETE FROM arrime_fincas`);
      
      let registrosRestored = 0;
      let centralesRestored = 0;
      let fincasRestored = 0;
      
      for (const c of datos.centrales || []) {
        await db.execute(sql`
          INSERT INTO arrime_centrales (id, nombre, color, orden)
          VALUES (${c.id}, ${c.nombre}, ${c.color}, ${c.orden})
          ON CONFLICT (id) DO NOTHING
        `);
        centralesRestored++;
      }
      
      for (const f of datos.fincas || []) {
        await db.execute(sql`
          INSERT INTO arrime_fincas (id, nombre, orden)
          VALUES (${f.id}, ${f.nombre}, ${f.orden})
          ON CONFLICT (id) DO NOTHING
        `);
        fincasRestored++;
      }
      ...
      for (const r of datos.registros || []) {
        await db.execute(sql`
          INSERT INTO arrime_registros (id, fecha, central, cantidad, grado, finca, remesa)
          VALUES (${r.id}, ${r.fecha}, ${r.central}, ${r.cantidad}, ${r.grado}, ${r.finca}, ${r.remesa})
        `);
        registrosRestored++;
      }
      ...
      res.json({
        success: true,
        restored: {
          registros: registrosRestored,
          centrales: centralesRestored,
          fincas: fincasRestored
        }
      });
    } catch (error) {
      console.error("Error al restaurar backup de arrime:", error);
      res.status(500).json({ error: "Error al restaurar backup" });
    }
  });

  app.delete("/api/arrime/backups/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const result = await db.execute(sql`DELETE FROM arrime_backups WHERE id = ${id} RETURNING id`);
      if (result.rows.length === 0) {
        return res.status(404).json({ error: "Backup no encontrado" });
      }
      res.status(204).send();
    } catch (error) {
      console.error("Error al eliminar backup de arrime:", error);
